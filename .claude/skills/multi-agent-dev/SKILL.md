...
description: Execute a 6-stage multi-agent development workflow (Product Manager/Project Manager → Tech Lead → Senior Dev → Developer → Reviewer → Integrator). Use for any software development task: features, refactoring, bug fixes, or new projects. Produces PRODUCT_BRIEF.md, PRD.md, PLAN.md, DESIGN.md, REVIEW.md artifacts. Supports [quick] mode, [medium] mode, single-stage execution, and auto-loop review cycles. Features AskUserQuestion integration at key decision points.
---
...
/multi-agent-dev [medium] <任务描述>  # 中等模式：PM → Tech Lead → Dev → Integrator（4阶段）
/multi-agent-dev [quick] <任务描述>   # 轻量模式：Dev → Integrator（2阶段，含 mini-design）
...
| 中等复杂度、需求较明确、有现有模式可参考 | `[medium]` 中等模式（4阶段） |
...
### 中等模式 `[medium]`（4 阶段）NEW

```
Stage 0: Product Manager  ──→  PRODUCT_BRIEF.md（简化版，跳过完整项目章程）
Stage 1: Tech Lead        ──→  PLAN.md（含 AskUserQuestion 需求确认）
Stage 3: Developer        ──→  代码实现 + 本地验证
Stage 5: Integrator       ──→  集成验证（含内嵌 Review）
```

适用：中等复杂度功能、需求较明确但需要技术方案把关、有现有模式可参考的改动。
...
1. **需求确认（CRITICAL — 必须使用 AskUserQuestion）**：使用 `AskUserQuestion` 工具进行三轮结构化询问，替换旧的文本 "回复 Y" 模式：
...
2. **设计权衡确认（CRITICAL — 使用 AskUserQuestion）**：当存在多个合理设计方案时，使用 `AskUserQuestion` 工具询问用户关键设计决策的偏好（如：组件拆分粒度、抽象层级、状态管理方案）。每个选项附带利弊说明。如果设计方向明确无争议，可跳过此步。
3. **允许修订 PLAN.md**：如发现 PLAN.md 中的问题（遗漏的边界情况、错误的接口签名、不合适的技术选型等），直接修改 PLAN.md 并在修改处标注 `[SENIOR-DEV AMENDMENT: <原因>]`。Developer 应以修订后的 PLAN.md 为准。
...
**检查点（NEEDS_DISCUSSION → 暂停，使用 AskUserQuestion）：**

当 NEEDS_DISCUSSION 触发时，编排者必须使用 `AskUserQuestion` 工具询问用户：

```json
{
  "question": "审查循环 3 次后仍有 N 个 BLOCKER 未解决。未解决的 BLOCKER：\n\n<列出每个 BLOCKER 及风险等级>\n\n请选择处理方式：",
  "header": "阻塞处理",
  "options": [
    {
      "label": "接受已知风险",
      "description": "标记为已知问题，继续进入集成阶段。适合低风险或边缘场景的 BLOCKER。"
    },
    {
      "label": "手动修复",
      "description": "我会手动修复这些问题，修复后重新触发审查（/multi-agent-dev stage 4）。"
    },
    {
      "label": "缩小范围",
      "description": "回退有问题的改动部分，仅保留通过审查的变更进入集成。"
    }
  ]
}
```
...
17. **新项目继承**：新项目初始化时自动继承全局注册表中的 [global] 模式，以及匹配当前技术栈的 [stack] 模式。项目可覆盖但不可删除继承的模式。
